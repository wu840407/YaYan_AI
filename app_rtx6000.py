...
DIALECT_TO_ROUTING = {
    "🔍 自動偵測（建議）": "auto",
    # 這兩項都走 auto。V5.2.0 起自動偵測能分辨閩南語/粵語/吳語/湘語/西南官話，
    # 會自動啟用對應的專用引擎（實測閩南語 94%、粵語 92%）。
    # ⚠️ 客家話、贛語、維吾爾語沒有對應標籤，自動偵測永遠判不出來，必須直接指定。
                ...
                with gr.Row():
                    with gr.Column(scale=1):
                        ...
                        dialect = gr.Dropdown(
                            ...
                            value="🔍 自動偵測（建議）",
                            label="來源語言",
                            info="一般情況建議維持「自動偵測」：系統會逐段判斷語言，"
                                 "閩南語／台語、粵語等會自動送到該語言的專用辨識引擎。"
                                 "已確定來源語言時直接指定，結果最穩定；"
                                 "客家話、贛語、維吾爾語無法自動偵測，請直接指定。",
                        )
